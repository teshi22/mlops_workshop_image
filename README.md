...
(Recommend) For Step 3, add `input_train_data_dir` and `input_valid_data_dir` properties that mean csv file path.
...
```bash
az ml online-endpoint create -f ./05_deploy/online-endpoint.yml 
az ml online-deployment create -n version-01 --endpoint vgg16-endpoint -f ./05_deploy/online-deployment.yml
```

and you can test API with ./05_deploy/test-endpoint.ipynb